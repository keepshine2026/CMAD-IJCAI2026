...
from CMAD.utils.const import CMAD_ROOT
...
def Youtube(url, has_subtitles):
    ...
    if has_subtitles:
        ...
        video_stream.download(output_path=str(CMAD_ROOT / "workspace"), filename=f"{video_id}.mp4")
        print('Video downloaded successfully')
        return str(CMAD_ROOT / "workspace" / f"{video_id}.mp4")
    else:
